...
class SchoolView extends React.PureComponent {
  ...
  handleDelete = ((key) => {
    ...
    this.setState({ 
      ...
      console.log(this.state, key,  dataSource.filter(item => item.id != key) )
    });
  }); 
  handleAllDelete =(() =>{
    const dataSource = [...this.state.dataSource];
    ...
    // handleAllSearch = (dataIndex,selectedKeys, confirm) =>{ {
    //   confirm();
      ...
      <div className="school_message">
        <Card title="学校管理" className="school_card">
          <div>
            ...
            // onPressEnter={() => this.handleSearch(selectedKeys, confirm)}
            // onSearch={this.handleAllSearch('schoolName',selectedKeys, confirm)}
            enterButton />
            <Button
            type="primary"
