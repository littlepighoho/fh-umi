...
class SchoolView extends React.PureComponent {
  ...
  componentDidMount() {
    ...
    //   },
    // ]
    console.log("list:&&&&&&&"+get(this.props,"list.pagination.total",""));
  }
    ...
    this.setState({
        ...
        // 新建
    handleAdd = (e) => {
      const data =get(this.state.school,"entities",[]);
        const { count } = this.state;
        e.preventDefault();
        this.props.data=(err, values) => {
        if (!err) {
            const newData = {
                id:count,
                key:count,
                ...
                update_time:formateDate(new Date().getTime()),
                create_time:formateDate(new Date().getTime()),
            };
            this.setState({
    ...
    handleCreate = (e) => {
        ...
        this.props.form.validateFields((err, values) => {
          console.log(values.description);
            ...
            if(values.id)
              data.schoolId = values.id;
            if(values.name)
              data.name = values.name;
            if(values.description)
              data.description = values.description;
            if(data.create_time == null){
               data.create_time = formateDate(new Date().getTime());
               data.update_time = formateDate(new Date().getTime());
            }
            else
              data.update_time = formateDate(new Date().getTime());
            console.log(formateDate(new Date().getTime()));
            this.props.form.resetFields();
            this.setState({
                visible1: false,
            });
      ...
      <div className="student_message">
        <Card title="学校管理" className="student_card">
          ...
          <Table
            ...
            pagination= {{
              // page:get(this.props,"list.pagination.page",""),
              total:get(this.props,"list.pagination.total",""),
              pageSize:get(this.props,"list.pagination.limit",""),
            }}
          >
        ...
        >
            <Form onSubmit={this.handleAdd}  layout="vertical" hideRequiredMark style={{width:'300px'}}>
                    ...
                    {getFieldDecorator('id',{
                            rules:[{required:true,message:'学校id不能为空'}],
                        })(
        ...
        >
          <Form layout="vertical">
            ...
            {getFieldDecorator('id',{
                    ...
                    <Input  placeholder={this.state.editData.schoolId}/>
                )}
            </Form.Item>
            <Form.Item label="学校名称">
                    {getFieldDecorator('name', {
